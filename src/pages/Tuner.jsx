...
function freqToNote(freq) {
  ...
  const octave = Math.floor(rounded / 12) + 4;
  const cents = (noteNum - rounded) * 100;
  const idealFreq = 440 * Math.pow(2, rounded / 12);
  return { note, octave, cents, idealFreq, freq };
}
...
export default function Tuner() {
  ...
  const closestString = noteInfo
    ? GUITAR_STRINGS.reduce((best, s) => {
        const d = Math.abs(s.freq - noteInfo.freq);
        return d < Math.abs(best.freq - noteInfo.freq) ? s : best;
      }, GUITAR_STRINGS[0])
    : null;
    ...
    <div>
      ...
      <div className="card" style={{ maxWidth: 520, margin: '0 auto' }}>
        ...
        <div className="tuner-display">
          <div
            className={`tuner-note${inTune && noteInfo ? ' in-tune' : ''}`}
            style={{ color: noteInfo ? (inTune ? 'var(--green)' : cents < 0 ? 'var(--red)' : 'var(--gold)') : 'var(--text-3)' }}
          >
            {noteInfo?.note || '—'}
          </div>
          ...
          <div style={{ maxWidth: 360, margin: '28px auto 0' }}>
            ...
            <div className="tuner-meter">
              <div style={{
                width: '100%',
                height: '100%',
                background: `linear-gradient(90deg, var(--red), var(--green) 50%, var(--gold))`,
                opacity: 0.3,
                borderRadius: 4,
              }} />
            </div>
            <div style={{ position: 'relative', height: 20, marginTop: -14 }}>
              <div style={{
                ...
                boxShadow: `0 0 12px ${inTune && noteInfo ? 'var(--green)' : 'var(--accent-glow)'}`,
          ...
          {noteInfo && (
            <div style={{
              ...
              fontSize: 13,
              fontWeight: 700,
              color: inTune ? 'var(--green)' : cents < 0 ? 'var(--red)' : 'var(--gold)',
            }}>
